...
def pxrd():
    file = pd.ExcelFile('pxrd.xlsx')
    df = file.parse('I2θ')

    I = df['I'].values      # Intensity value
    2θ = df[''].values      # θ values = df['2θ'].values   # 2θ values
    θ = df[''].values       # θ values

    rad = [i * (np.pi / 180) for i in θ]  # Conversion of degree to radian
    ...
    λ = 0.1540  # Values in nm for CuKα
    D = (0.9 * λ)/(β * cosθ)



# DISLOCATION DENSITY, δε
δ = 15ε/aD # a = lattice constant
...
def Kinetics():
    ...
    for x in range(len(qe_qt)):
        print("%s%s%s%s%s%s%s%s%s%s%s" % (
            format((x + 1),'^4'), format(Time[x],'>9'), format(format(rem_eff[x],".3f"),'>14'), format(format(ce[x],".3f"),'>12'), format(format(qe[x],".3f"),'>12'), format(format(logqe_qt[x],".3f"),'>14'),
                format(format(logqe[x],".3f"),'>13'), format(format(tbyqt[x],".3f"),'>12'), format(format(log_t[x],".3f"),'>12'), format(format(ln_t[x],".3f"),'>10'), format(format(root_t[x],".3f"),'>15')))
        x = x + 1
    print("----------------------------------------------------------------------------------------------------------------------------------")
...
def Option():
    print("Press 1 for ISOTHERM Results")
    print("Press 2 for KINETICS Results:")
    print("Press 3 for THERMODYNAMICS Results")
